...
function Search() {
  ...
  useEffect(() => {
    ...
    return () => document.removeEventListener("keyup", searchKeyPressHandler) //returning cleanup function
  }, [])
  ...
  useEffect(() => {
    if (state.searchTerm.trim()) {
      ...
      return () => clearTimeout(delay) //cleanup function. This function executes not only when component unmount, but every time when useEffect executes
    } else {
      setState(draft => {
        draft.show = "neither"
      })
